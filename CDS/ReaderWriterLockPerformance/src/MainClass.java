...
public class MainClass {
	...
	public static void main(String[] arg) {
		...
		for (int i = 0; i < 2; i++) {
			WriterClient writerClient = new WriterClient(timeSlicer,
					readWriter, 1400);
		...
		while (true) {
			try {
				...
				Thread.sleep(1000);
				// Set a boolean to true, which is referred by all the Threads
				// in their run method
				timeSlicer.freezeTime();
				/*if(System.currentTimeMillis() - startTime > 5000){
					System.out.println("Successful Read :" + FifoReadWriterLock.successfulRead + "Successful Write :" + FifoReadWriterLock.successfulWrite);
				...
				for (int j = 0; j < threadArray.length; j++) {
					...
					if (threadArray[j] instanceof Client)
						System.out.print(((Client) threadArray[j]).getName()
								+ " : " + ((Client) threadArray[j]).isActive
								+ " | ");
				}
