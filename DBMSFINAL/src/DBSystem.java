...
public class DBSystem {
	...
	public void optimizeJoin() {
		TableDetails dual = null;
		List sizeOfRelation = new ArrayList<>();
		for(int i=0; i < _select_targetTable_List.size(); i ++){
			String table = _select_targetTable_List.get(i).toString();
			dual = DBSystem.allTables.get(table);
			sizeOfRelation.add(dual.getNumberOfRecords());
			System.out.println("Size of Relation " + table + " is : " + dual.getNumberOfRecords());
			//_tableList.get(_select_targetTable_List.get(1))
		}
		
		TableDetails table1,table2=null;
		int lowestCost = Integer.MAX_VALUE;
		List bestCostJoinOrder = null;
		for(int a = 0; a < _select_targetTable_List.size(); a++){
			String table1_Name = _select_targetTable_List.get(a).toString();
			table1 = allTables.get(table1_Name);
			for(int b = a+1; b < _select_targetTable_List.size(); b++ ){
				String table2_Name = _select_targetTable_List.get(b).toString();
				table2 = allTables.get(table2_Name);
				for(int x =0; x < _join_Left_Cond.size(); x++){
					String leftCond = _join_Left_Cond.get(x).toString();
					String rightCond = _join_Right_Cond.get(x).toString();
					String[] leftCondSplit = leftCond.split("_");
					String[] rightCondSplit = rightCond.split("_");
					if((table1_Name.equalsIgnoreCase(leftCondSplit[0]) && table2_Name.equalsIgnoreCase(rightCondSplit[0]))){
						int v1_index = Integer.parseInt(leftCondSplit[1]);
						int v1 = ((HashMap)table1.getIndex().get(v1_index)).size();
						int v2_index = Integer.parseInt(rightCondSplit[1]);
						int v2 = ((HashMap)table2.getIndex().get(v2_index)).size();
						int cost = (table1.getNumberOfRecords() * table2.getNumberOfRecords())/Math.max(v1, v2);
						if(cost < lowestCost){
							lowestCost = cost;
							bestCostJoinOrder = null;
							bestCostJoinOrder = new ArrayList<>();
							//bestCostJoinOrder = "(" +table1_Name + " INNER JOIN " + table2_Name + ")";
							bestCostJoinOrder.add(table1_Name);
							bestCostJoinOrder.add(table2_Name);
						}	
						System.out.println("Cost of Join " + table1_Name + " INNER JOIN " + table2_Name + " is : " + "(" + table1.getNumberOfRecords() + "*" + table2.getNumberOfRecords() + ") / " + "MAX(" + v1+","+v2 +") = " + cost);
						
					} else if((table1_Name.equalsIgnoreCase(rightCondSplit[0]) && table2_Name.equalsIgnoreCase(leftCondSplit[0]))) {
						int v1_index = Integer.parseInt(rightCondSplit[1]);
						int v1 = ((HashMap)table1.getIndex().get(v1_index)).size();
						int v2_index = Integer.parseInt(leftCondSplit[1]);
						int v2 = ((HashMap)table2.getIndex().get(v2_index)).size();
						int cost = (table1.getNumberOfRecords() * table2.getNumberOfRecords())/Math.max(v1, v2);
						System.out.println("Cost of Join " + table1_Name + " INNER JOIN " + table2_Name + " is : " + "(" + table1.getNumberOfRecords() + "*" + table2.getNumberOfRecords() + ") / " + "MAX(" + v1+","+v2 +") = " +  cost);
						if(cost < lowestCost){
						...
						int cost = (table1.getNumberOfRecords() * table2.getNumberOfRecords());
						System.out.println("Cost of Join " + table1_Name + " INNER JOIN " + table2_Name + " is : " + "(" + table1.getNumberOfRecords() + "*" + table2.getNumberOfRecords() + ")  = " +  cost);
						if(cost < lowestCost){
							lowestCost = cost;
							bestCostJoinOrder = null;
							bestCostJoinOrder = new ArrayList<>();
		...
		_select_targetTable_List.removeAll(bestCostJoinOrder);
		System.out.println("Lowest cost Join Order : (("+ bestCostJoinOrder.get(0) + " INNER JOIN " + bestCostJoinOrder.get(1) + ")" + " INNER JOIN " + _select_targetTable_List.get(0) + ")");
	}
				...
				if(index_right.containsKey(key)){
					...
					for(int i = 0; i < leftList.size(); i++){
						...
						for(int j = 0; j < rightList.size(); j++){
							...
							right_record_array = right_record.split(",");
	...
	public void executeSelectCommand(TSelectSqlStatement selectStmt) {
		...
		List<Integer> tentativeRecords = getIntentiveRecords();
		...
		try {
			...
			//System.out.println("Using Liner Scan:");
			//for (int i = 0; i < totalRecordsInTable; i++) {
				//String record = getRecord(_select_TargetTable, i);
			System.out.println("Using Indexing:");
			for (int i = 0; i < tentativeRecords.size(); i++) {
				String record = getRecord(_select_TargetTable, tentativeRecords.get(i));
	...
	public void executeSelectWithSortMerge(){
		// Only two tables
		//for(int i =0; i < _join_Left_Cond.size(); i++) {
			String[] table1_colIndex = _join_Left_Cond.get(0).toString().split("_");
			sortTable(table1_colIndex[0], Integer.parseInt(table1_colIndex[1]), "t" + 0 + "_");
			String[] table2_colIndex = _join_Right_Cond.get(0).toString().split("_");
			sortTable(table2_colIndex[0], Integer.parseInt(table2_colIndex[1]), "t" + 1 + "_");
		//}
		
		BufferedReader b1reader = null,b2reader = null;
		BufferedWriter fos = null;
		try {
			b1reader = new BufferedReader(new FileReader("./join" + orderByQueryCount + "/" + table1_colIndex[0] + "_sorted.csv"));
		...
		//String right_record = b2reader.readLine();
		String[] left_record_array = null;
		String[] right_record_array = null;
		String tableName = null;
		String[] table_col_left = _join_Left_Cond.get(0).toString().split("_");
		String[] table_col_right = _join_Right_Cond.get(0).toString().split("_");
		
		try {
			String left_record = b1reader.readLine();
			//b2reader.mark(0);
			while(left_record != null){
				//left_record = b1reader.readLine();
				left_record_array = left_record.split(",");
				b2reader = new BufferedReader(new FileReader("./join" + orderByQueryCount + "/" + table2_colIndex[0] + "_sorted.csv"));
				//if(b2reader.toString() != null)
					//b2reader.reset();
				String right_record = b2reader.readLine();
				
				while(right_record != null){
					right_record_array = right_record.split(",");
					int value = (left_record_array[Integer.parseInt(table_col_left[1])]).toLowerCase().compareTo((right_record_array[Integer.parseInt(table_col_right[1])]).toLowerCase());
					right_record = b2reader.readLine();
					//equals
					if(value == 0){
						for( int d = 0; d < select_Column_Order_List.size(); d++){
							tableName = select_Column_Order_Table_List.get(d).toString();
	...
	public void sortTable(String tableName, int sortColIndex, String chunkPrefix){
		TableDetails dual = DBSystem.allTables.get(tableName);
		int totalRecordsInTable = dual.getNumberOfRecords();
		...
		try {
			...
			fos = new BufferedWriter(new FileWriter("./join" + orderByQueryCount + "/" + chunkPrefix +  numOfFiles + ".csv", true));
			
			List<RecordLine> listOfLines = new ArrayList<RecordLine>();

			for (int i = 0; i < totalRecordsInTable; i++) {
				String record = getRecord(tableName, i);
				String[] cols = record.split(",");
				if(cols.length <= 2)
					continue;
				for (int a = 0; a < cols.length; a++) {
				...
				if (fileSize + record.getBytes().length > _pageSize) {
					writeFromListToFileSorted(fos, listOfLines, sortColIndex, tableName);
					fos.flush();
					fos.close();
					numOfFiles++;
					fos = new BufferedWriter(new FileWriter("./join"  + orderByQueryCount + "/" + chunkPrefix + numOfFiles + ".csv", true));
				...
				//if(numOfFiles > 0){
				mergeTableSort(numOfFiles, sortColIndex, tableName, chunkPrefix);
				//}
	 ...
	 * @param numOfFiles
	 */
		private void mergeTableSort(int numOfFiles, int sortColIndex, String tableName, String chunkPrefix) {
			try {
				...
				for (int index = 0; index <= numOfFiles; index++) {
					String fileName = "./join" + orderByQueryCount + "/"+ chunkPrefix + index + ".csv";
					listOfFileReader.add(new FileReader(fileName));
					listOfBufferedReader.add(new BufferedReader(listOfFileReader.get(index)));
				}
				
				sortTableAndWriteOutput(listOfBufferedReader, sortColIndex, tableName);
		 ...
		 */
		private void sortTableAndWriteOutput(List<BufferedReader> listOfBufferedReader, int sortColIndex, String fileName) {
			try {
				...
				FileWriter fw = new FileWriter("./join" + orderByQueryCount + "/"+ fileName + "_sorted.csv");
				BufferedWriter bw = new BufferedWriter(fw);
				while (true) {
					if (listOfLinesfromAllFiles.size() == 0) {
						break;
					} else {
						Collections.sort(listOfLinesfromAllFiles, new RecordLine(sortColIndex,fileName));
						...
						if (line != null) {
							...
							for (int index = 0; index < listOfBufferedReader.size(); index++) {
								if (index == indexForFileName) {
									...
									line = listOfBufferedReader.get(index).readLine();
									if (line != null) {
										listOfLinesfromAllFiles.add(new RecordLine(line, index, sortColIndex));
										continue;
									}
	 ...
	 * @throws IOException
	 */
	private void writeFromListToFileSorted(BufferedWriter fos, List<RecordLine> listOfLines, int sortColIndex, String tableName) throws IOException {
		Collections.sort(listOfLines, new RecordLine(sortColIndex,tableName));		
	 ...
	 * @return
	 */
	private int writeToList(int fileSize, List<RecordLine> listOfLines,	String record, int numOfFiles, int sortColIndex) {
 ...
 */
	...
	public List<Integer> getIntentiveRecords() {
		...
		for (int i = 0; i < _select_where_Column.size(); i++) {
			...
			List values = (List)colMap.get(_select_where_Column_Value.get(i));
			for(int j = 0; j < values.size(); j++){
				returnList.add((int)values.get(j));
			}
